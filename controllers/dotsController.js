...
const getDotsBySiteName = async (req, res, next) => {
  try {
    ...
    res.status(200).json(siteDots.rows);
  } catch (error) {
    next(error);
  }
};
...
const createDot = async (req, res, next) => {
  try {
    ...
    const createDot = await dotQ.createDot(
      longitude,
      latitude,
      rsrp,
      site_name
    );
    res.status(200).json(createDot);
  } catch (error) {
    next(error);
  }
};
...
const updateDot = async (req, res, next) => {
  try {
    const { id } = req.params;
    const { longitude, latitude, rsrp, site_name } = req.body;
    const newDot = await dotQ.updateDot(
      id,
      longitude,
      latitude,
      rsrp,
    ...
    res.status(200).json('Dot have been updated');
  } catch (error) {
    next(error);
  }
};
...
const deleteAllRows = async (req, res, next) => {
  try {
    ...
    res.status(200).json('All Rows Deleted');
  } catch (error) {
    next(error);
  }
};
...
const getAllDistinct = async (req, res, next) => {
  try {
    ...
    res.status(200).json(allDistinct);
  } catch (error) {
